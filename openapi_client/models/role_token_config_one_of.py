...
class RoleTokenConfigOneOf(BaseModel):
    """
    RoleTokenConfigOneOf
    """
    address: StrictStr = Field(..., description="A cardano address, in Bech32 format")
    metadata: TokenMetadata = Field(...)
    __properties = ["address", "metadata"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RoleTokenConfigOneOf:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> RoleTokenConfigOneOf:
        """Create an instance of RoleTokenConfigOneOf from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RoleTokenConfigOneOf.parse_obj(obj)

        _obj = RoleTokenConfigOneOf.parse_obj({
